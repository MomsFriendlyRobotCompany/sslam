/*
c_sslam.c : C extensions for sslam in Python
...
static PyTypeObject c_sslam_PositionType =
{
...
PyObject * Position_copy(Position *self, PyObject *args, PyObject *kwds)
{
	...
	PyObject_CallObject((PyObject *) &c_sslam_PositionType, argList);
	Py_DECREF(argList);
...
static void Scan_dealloc(Scan* self)
{
	scan_free(&self->scan);
...
static PyObject *Scan_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
	Scan *self;
...
static int Scan_init(Scan *self, PyObject *args, PyObject *kwds)
{
...
static PyObject *Scan_str(Scan *self)
{
	char scanstr[100];
...
static PyObject *Scan_update(Scan *self, PyObject *args, PyObject *kwds)
{
...
static PyTypeObject c_sslam_ScanType =
{
	#if PY_MAJOR_VERSION < 3
	PyObject_HEAD_INIT(NULL)
	0,                                          // ob_size
...
static PyObject *Map_str(Map * self)
{
	char mapstr[100];
...
static PyObject *Map_get(Map * self, PyObject * args, PyObject * kwds)
{
...
static PyObject *Map_update(Map *self, PyObject *args, PyObject *kwds)
{
	Scan * py_scan = NULL;
	...
	if (error_on_check_argument_type((PyObject *)py_scan, &c_sslam_ScanType, 0,
			"c_sslam.Scan", "Map", "update") ||
		error_on_check_argument_type((PyObject *)py_position, &c_sslam_PositionType, 0,
			"c_sslam.Position", "Map", "update"))
	{
		return NULL;
	}
...
static PyTypeObject c_sslam_MapType =
{
	#if PY_MAJOR_VERSION < 3
	PyObject_HEAD_INIT(NULL)
	0,                                          // ob_size
...
static void Randomizer_dealloc(Randomizer* self)
{
...
static PyObject *Randomizer_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
	Randomizer *self;
...
static int Randomizer_init(Randomizer *self, PyObject *args, PyObject *kwds)
{
	int seed;
...
static PyTypeObject c_sslam_RandomizerType =
{
	#if PY_MAJOR_VERSION < 3
	PyObject_HEAD_INIT(NULL)
	0,                                          // ob_size
...
static PyObject *distanceScanToMap(PyObject *self, PyObject *args)
{
	Map * py_map = NULL;
	Scan * py_scan = NULL;
	Position * py_position = NULL;
	...
	{
		return null_on_raise_argument_exception("sslam", "distanceScanToMap");
	}

	// Check object types
	if (error_on_check_argument_type((PyObject *)py_map, &c_sslam_MapType, 0,
			"c_sslam.Map", "c_sslam", "distanceScanToMap") ||
		error_on_check_argument_type((PyObject *)py_scan, &c_sslam_ScanType, 1,
			"c_sslam.Scan", "c_sslam", "distanceScanToMap") ||
		error_on_check_argument_type((PyObject *)py_position, &c_sslam_PositionType, 2,
			"c_sslam.Position", "c_sslam", "distanceScanToMap"))
	{
			return NULL;
	}
...
static PyObject *rmhcPositionSearch(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args, "OOOOddiO",
		...
		&py_randomizer))
	{
		return null_on_raise_argument_exception("sslam.algorithms", "rmhcPositionSearch");
	}
...
static PyMethodDef module_methods[] =
{
	...
	"map is a sslam.components.Map object\n"\
	"scan is a sslam.components.Scan object\n"\
	"position is a sslam.components.Position object\n"\
	},
...
{
	add_class(module, &c_sslam_ScanType, "Scan");
	add_class(module, &c_sslam_MapType, "Map");
	add_class(module, &c_sslam_PositionType, "Position");
	add_class(module, &c_sslam_RandomizerType, "Randomizer");
}

static int types_are_ready(void)
{
return
	type_is_ready(&c_sslam_ScanType) &&
	type_is_ready(&c_sslam_MapType) &&
	type_is_ready(&c_sslam_PositionType) &&
	type_is_ready(&c_sslam_RandomizerType);
}

#if PY_MAJOR_VERSION < 3

PyMODINIT_FUNC initc_sslam(void)
{
	if (!types_are_ready())    {
		return;
	}

	PyObject * module = Py_InitModule("c_sslam", module_methods);
