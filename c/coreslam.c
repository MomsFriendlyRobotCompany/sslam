...
static int out_of_bounds(int value, int bound)
{
...
static int clip(int *xyc, int * yxc, int xy, int yx, int map_size)
{
...
void map_string(map_t map, char * str)
{
...
void map_get(map_t * map, char * bytes)
{
    int k;
...
void map_set(map_t * map, char * bytes)
{
    int k;
    for (k=0; k<map->size_pixels*map->size_pixels; ++k)
    {
...
void scan_string(scan_t scan, char * str)
{
...
position_t rmhc_position_search(
